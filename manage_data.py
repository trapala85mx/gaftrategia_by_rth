...
async def analyze_sell_points(order_book: OrderBook, sl: float) -> list:
    ...
    if (order_book.sp_v_m1 > order_book.sp_v_m2) and (order_book.sp_c_m1 > 0 and order_book.sp_c_m2 > 0):
        pct_ventas = round(
...
async def signal(sell_data_points:list, buy_data_points:list, order_book:OrderBook):
    ...
    if sell_data_points[0] == 0 and buy_data_points[0] > 0:
        #señal de compra
        print(f"{'*' * 40}")
        print(f"Señal de Compra en {order_book.symbol}")

    if sell_data_points[0] > 0 and buy_data_points[0] > 0:
        #señal de venta
        print(f"{'*' * 40}")
        print(f"Señal de Venta en {order_book.symbol}")

    if sell_data_points[0] > 0 and buy_data_points[0] > 0:
        #señal de venta y compra
        print(f"{'*' * 40}")
        print(f"Señal de Venta y Compra en {order_book.symbol}")
...
async def manage_data(order_book: OrderBook):
    while True:
        if order_book.price > 0 and order_book.sp_v_m1 > 0:
            ...
            sign = await signal(sell_data_points, buy_data_points, order_book)
